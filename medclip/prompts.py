...
def generate_class_prompts(df_sent, task=None, n=100):
    ...
    for task in target_tasks: # No Finding
        other_tasks = [t for t in all_tasks if t != task] # df_sub_sent
...
def generate_covid_class_prompts(n = None):
    ...
    for k, v in constants.COVID_CLASS_PROMPTS.items():
        cls_prompts = []
        keys = list(v.keys()) # ['adjective', 'description', 'subtype', 'location']
        ...
        # ['patchy ground glass opacity in peripheral', 'patchy ground glass opacity in mid', 'patchy ground glass opacity in lower', 'patchy ground glass consolidation in peripheral', 'patchy ground glass consolidation in mid', 'patchy ground glass consolidation in lower', 'confluent ground glass opacity in peripheral', 'confluent ground glass opacity in mid', 'confluent ground glass opacity in lower', 'confluent ground glass consolidation in peripheral', 'confluent ground glass consolidation in mid', 'confluent ground glass consolidation in lower']
        ...
        if n is not None and n < len(cls_prompts):
            ...
            prompts[k] = cls_prompts
        print(f'sample {len(prompts[k])} num of prompts for {k} from total {len(cls_prompts)}')
        # {'COVID': ['confluent ground glass consolidation in mid', 'patchy ground glass opacity in mid', 'patchy ground glass opacity in peripheral', 'patchy ground glass consolidation in mid', 'patchy ground glass consolidation in peripheral', 'confluent ground glass consolidation in lower', 'confluent ground glass opacity in peripheral', 'confluent ground glass consolidation in peripheral', 'confluent ground glass opacity in lower', 'patchy ground glass opacity in lower']}
    return prompts
