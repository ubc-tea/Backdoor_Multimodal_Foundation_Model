...
class MedCLIPModel(nn.Module):
    def __init__(self,
        ...
        if checkpoint is not None:
            state_dict = torch.load(os.path.join(checkpoint))
            self.load_state_dict(state_dict)
            print('load model weight from:', checkpoint)
        
        if mode == 'train':
            self.forward = self.forward_all
        elif mode == 'backdoor_image':
            self.forward = self.encode_image
        else:
    ...
    def compute_logits(self, img_emb, text_emb): # [batch_size, 512] [batch_size, 512]
